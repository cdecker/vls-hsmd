...
from utils import (
    TIMEOUT, first_scid, GenChannel, generate_gossip_store, wait_for, VLS_MODE_NATIVE
)
...
@unittest.skipIf(
    not VLS_MODE_NATIVE,
    "msat amount is > I32_MAX, which cannot be parsed by VLS (?)"
)
...
@unittest.skipIf(not VLS_MODE_NATIVE, "fails in hsmd_sign_b12")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "VLS fails to parse 4294967295 (?)")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Remote configuration not allowed in VLS modes.")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "VLS seems unable to parse msat amounts > INT_MAX")
