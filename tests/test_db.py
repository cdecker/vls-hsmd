...
@unittest.skipIf(os.getenv('VLS_MODE') != 'cln:native', "remote_hsmd doesn't like channel_nonce changing")
...
@unittest.skipIf(TEST_NETWORK != 'regtest', "The network must match the DB snapshot")
@unittest.skipIf(
    os.getenv('VLS_MODE') != 'cln:native',
    "VLS does not have a matching state for the loaded DB, causing it to reject everything."
)
...
@unittest.skipIf(
    os.getenv('VLS_MODE') != 'cln:native',
    "VLS does not work with pre-existing DBs, no matching signer state"
)
...
@unittest.skipIf(os.getenv('VLS_MODE') != 'cln:native', "vlsd chokes on allowlist when started on wrong network")
...
@unittest.skipIf(
    os.getenv('VLS_MODE') != 'hsmd:remote_hsmd',
    "Loading an existing DB for which the signer has no matching state."
)
