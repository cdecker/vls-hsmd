...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "Negotiation does not resume, a unilateral close is being triggered.")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "handle_sign_option_will_fund_offer unimplemented")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "invalid next holder commitment number: 4 != 6")
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "Makes use of the sqlite3 db")
@pytest.mark.slow_test
@pytest.mark.parametrize("anchors", [False, True])
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native" and os.getenv('VLS_PERMISSIVE') != '1', "exceeds max fee policy")
@pytest.mark.parametrize("anchors", [False, True])
...
@pytest.mark.slow_test
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "frequently flakes")
...
@flaky
@pytest.mark.slow_test
@unittest.skipIf(not VLS_MODE_NATIVE, "Restart is causing vlsd2 to attempt to reopen an already open DB")
...
@unittest.skipIf(TEST_NETWORK != 'regtest', 'elementsd anchors not supportd')
@unittest.skipIf(not VLS_MODE_NATIVE, "Restart fails with port-collision for vlsd2")
