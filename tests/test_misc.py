...
from utils import (
    account_balance, scriptpubkey_addr, check_coin_moves, first_scid, VLS_MODE_NATIVE
)
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Path to binary is relative and doesn't resolve.")
...
@unittest.skipIf(
    not VLS_MODE_NATIVE,
    "The vlsd2 oracle is unaffected by the rpcproxy used to simulate errors, causing discrepancies between node and signer state."
)
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd integration test job control fails here")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "hsm_secret recovery not supported")
...
@unittest.skipIf(
    not VLS_MODE_NATIVE,
    "Problem with the last restart, blocking on VLS getting its port ready, despite receiving the hsmd_init message."
)
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Discrepancy between signer state and node state.")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Port collision")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken relative path to `lightningd`")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Fails to start up.")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd doesn't support forced secrets")
...
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken binary path.")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken path")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken relative path")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd doesn't create hsm_secret file")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd doesn't support dev-force-privkey")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd says no such channel")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "VLSD2 port conflict on restart.")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "remote_hsmd doesn't support this command")
@unittest.skipIf(os.getenv('TEST_DB_PROVIDER', 'sqlite3') != 'sqlite3', "deletes database, which is assumed sqlite3")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "needs VLS support VLS#497")
