...
from utils import (
    ...
    mine_funding_to_announce, VALGRIND, VLS_MODE_NATIVE
)
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken binary and plugin paths")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Broken plugin paths")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "this test frequently hangs w/ VLSD")
@pytest.mark.openchannel('v1')
@pytest.mark.openchannel('v2')
...
@unittest.skip("Plugin path is broken")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "channel too big, then feerate above maximum")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native" and os.getenv('VLS_PERMISSIVE') != '1', "dev_sign_last_tx causes subsequent validate_holder_commitment_tx failure")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "trouble managing remotesigner when node killed this way")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Plugin paths do not match due to linking of `contrib/plugins`")
...
@unittest.skipIf(not VLS_MODE_NATIVE, "Port conflict due to vlsd restart")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "timeout waiting for hold_invoice plugin")
...
@unittest.skipIf(os.getenv("VLS_MODE") != "cln:native", "exposesecret does not work with remote signer.")
